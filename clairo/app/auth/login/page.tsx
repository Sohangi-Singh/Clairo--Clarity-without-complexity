...
export default function LoginPage() {
  ...
  const handleGuestMode = () => {
    localStorage.setItem("clairo-mode", "guest");
    ...
    <div className="min-h-screen bg-[var(--bg-base)] flex items-center justify-center p-6">
      ...
      >
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-xl)] p-6 flex flex-col gap-4">
          ...
          <Input placeholder="Enter your email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} icon={<Mail size={16} />} />
          <Button variant="primary" fullWidth disabled={!email}>
            Send magic link <ArrowRight size={16} />
          </Button>
        </div>

        <div className="mt-4 text-center">
          <button onClick={handleGuestMode} className="text-[14px] text-[var(--accent)] hover:underline">
            Skip sign in — try as guest
          </button>
        </div>

        <p className="text-[12px] text-[var(--text-tertiary)] text-center mt-4">
          Don&apos;t have an account?{" "}
          <Link href="/auth/signup" className="text-[var(--accent)] hover:underline">Create one</Link>
        </p>
      </motion.div>
    </div>
  );
